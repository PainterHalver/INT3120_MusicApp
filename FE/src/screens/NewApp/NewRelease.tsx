...
export const NewRelease = memo(({releases}: Props) => {
    ...
    if (releases && releases.length > 0) {
      ...
      filterData =
        type === 1
          ? filterData
          : type === 2
          ? filterData.filter(item => (item.isWorldWide = false))
          : filterData.filter(item => (item.isWorldWide = true));
  ...
  const ColumnNewRelease = ({items}: {items: Release[]}) => {
    return useMemo(() => {
      return (
        <View style={{flexDirection: 'column', gap: 10, paddingTop: 10, width: screenWidth - 20}}>
          {items &&
            items.length > 0 &&
